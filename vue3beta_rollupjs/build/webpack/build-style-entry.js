...
function search(tree, checkList) {
  tree && Object.keys(tree).forEach(key => {
    ...
    const component = key.split(`${SEP}vx3-mobile-components${SEP}lib${SEP}`)[1].replace(`${SEP}index.js`, '').replace(`mixins${SEP}`, '')
...
function analyzeDependencies(componentName, libDir) {
  ...
  search(dependencyTree({
    ...
    filter: path => path.indexOf(`@csii/vx-mobile${SEP}lib${SEP}`) !== -1
  }), checkList)
