...
const mixins = [
  path.join(__dirname, '.src//components/_style/mixin/*.styl'),
...
gulp.task('compile', () => {
  const streamCompiledVue = gulp.src('./src/components/*/*.vue')
          .pipe(gulpVueify({
            extractCSS: true,
            customCompilers: {
  ...
  const streamCompiledCss = gulp.src('./src/components/*/*.vue')
          .pipe(gulpVueify({
            isExactCss: true,
            customCompilers: {
              stylus: compileVueStylus
  ...
  const streamCompiledStylus = gulp.src('./src/components/_style/*.styl')
          .pipe(gulpStylus({
            compress: true,
            import: mixins
          }))
  ...
  const streamCompiledJs = gulp.src('./src/components/**/!(test)/!(component).js')
          .pipe(preprocess({
            context: {
              'NODE_ENV': 'production',
