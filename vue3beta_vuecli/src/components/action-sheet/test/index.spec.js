import {ActionSheet} from '@csii/vx-mobile'
...
describe('ActionSheet - Operation', () => {
  ...
  test('empty options', () => {
    ...
    expect(wrapper.findAll('.ui-action-sheet-item').length).toBe(0)
  })
  ...
  test('cancel events', done => {
    ...
    setTimeout(() => {
      wrapper.find('.ui-action-sheet-cancel').trigger('click')
  ...
  test('disabled option', done => {
    ...
    setTimeout(() => {
      const disabledOption = wrapper.findAll('.ui-action-sheet-item').at(2)
  ...
  test('static methods create/closeAll', done => {
    ...
    setTimeout(() => {
      expect(document.body.querySelectorAll('.ui-action-sheet').length > 0).toBe(true)
