import {Captcha} from '@csii/vx-mobile'
...
describe('Captcha - Operation', () => {
  ...
  it('create a simple captcha', () => {
    ...
    expect(wrapper.classes('ui-captcha')).toBe(true)
  })
  ...
  it('create inline captcha', () => {
    ...
    expect(wrapper.contains('.ui-dialog')).toBe(false)
  })
  ...
  it('show and hide captcha', done => {
    ...
    setTimeout(() => {
      ...
      wrapper.find('.ui-dialog-close').trigger('click')
      wrapper.setProps({
        value: false,
      })
    }, 300)
  ...
  it('click button and emit send event', done => {
    ...
    const eventStub = sinon.stub(wrapper.vm, '$emit')
    setTimeout(() => {
      wrapper.find('.ui-captcha-btn').trigger('click')
  ...
  it('not countdown', () => {
    ...
    expect(wrapper.findAll('.ui-captcha-btn').length).toEqual(0)
  })
