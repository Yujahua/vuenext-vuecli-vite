import {Cashier} from '@csii/vx-mobile'
...
describe('Cashier - Operation', () => {
  ...
  it('cashier default-index', done => {
    ...
    setTimeout(() => {
      ...
      const channelItems = wrapper.findAll('.ui-cashier-channel-item')
      const confirmBtn = wrapper.find('.ui-cashier-pay-button')
  ...
  it('cashier captcha', done => {
    ...
    wrapper.vm.$nextTick(() => {
      expect(!!wrapper.find('.ui-cashier-captcha')).toBeTruthy()
      const cancel = wrapper.find('.ui-popup-close')
  ...
  it('cashier loading', done => {
    ...
    wrapper.vm.$nextTick(() => {
      expect(!!wrapper.find('.ui-cashier-loading')).toBeTruthy()
      expect(
        wrapper
          .find('.ui-cashier-block-text')
          .text()
          .trim(),
      ).toEqual('123')
      done()
  ...
  it('cashier success', done => {
    ...
    wrapper.vm.$nextTick(() => {
      expect(!!wrapper.find('.ui-cashier-success')).toBeTruthy()
      expect(
        wrapper
          .find('.ui-cashier-block-text')
          .text()
  ...
  it('cashier fail', done => {
    ...
    wrapper.vm.$nextTick(() => {
      expect(!!wrapper.find('.ui-cashier-fail')).toBeTruthy()
      expect(
        wrapper
          .find('.ui-cashier-block-text')
          .text()
