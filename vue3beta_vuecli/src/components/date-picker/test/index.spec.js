...
describe('DatePicker - Operation', () => {
  ...
  it('create a date picker', done => {
    ...
    wrapper.vm.$nextTick(() => {
      expect(wrapper.findAll('.ui-picker-column-item').length).toEqual(3)
      setTimeout(() => {
  ...
  it('create a time picker', done => {
    ...
    wrapper.vm.$nextTick(() => {
      expect(wrapper.findAll('.ui-picker-column-item').length).toEqual(2)
      done()
    })
  })
  ...
  it('create a datetime picker', done => {
    ...
    wrapper.vm.$nextTick(() => {
      expect(wrapper.findAll('.ui-picker-column-item').length).toEqual(5)
      done()
    })
  })
  ...
  it('create a popup picker', done => {
    ...
    wrapper.vm.$nextTick(() => {
      ...
      const cancel = wrapper.find('.ui-popup-cancel')
      cancel.trigger('click')
      setTimeout(() => {
        wrapper.setProps({value: true})
        const confirm = wrapper.find('.ui-popup-confirm')
