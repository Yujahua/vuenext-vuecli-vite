...
function coreMixin(BScroll) {
  ...
  BScroll.prototype._end = function(e) {
    ...
    if (
      ...
      var wrapperWidth =
        (this.directionX === DIRECTION_RIGHT && left) || (this.directionX === DIRECTION_LEFT && right)
          ? this.wrapperWidth
          : 0
      var wrapperHeight =
        (this.directionY === DIRECTION_DOWN && top) || (this.directionY === DIRECTION_UP && bottom)
          ? this.wrapperHeight
          : 0
      var momentumX = this.hasHorizontalScroll
        ? momentum(this.x, this.startX, duration, this.maxScrollX, this.minScrollX, wrapperWidth, this.options, this)
        : {destination: newX, duration: 0}
      var momentumY = this.hasVerticalScroll
        ? momentum(this.y, this.startY, duration, this.maxScrollY, this.minScrollY, wrapperHeight, this.options, this)
      ...
      if (this.options.wheel) {
        if (this.target && this.target.className === this.options.wheel.wheelWrapperClass) {
          ...
          var _offset = Math.round(
            (this.pointY + offsetToBody(this.wrapper).top - this.wrapperHeight / 2) / this.itemHeight,
          )
  ...
  BScroll.prototype._initSnap = function() {
    ...
    this.on('refresh', function() {
      ...
      if (snapThreshold % 1 === 0) {
        ...
        _this.snapThresholdX = Math.round(
          _this.pages[_this.currentPage.pageX][_this.currentPage.pageY].width * snapThreshold,
        )
        _this.snapThresholdY = Math.round(
          _this.pages[_this.currentPage.pageX][_this.currentPage.pageY].height * snapThreshold,
        )
      }
    })
    ...
    if (snap.listenFlick !== false) {
      this.on('flick', function() {
        var time =
          ...
          Math.max(
            Math.max(
              Math.min(Math.abs(_this.x - _this.startX), 1000),
              Math.min(Math.abs(_this.y - _this.startY), 1000),
            ),
            300,
          )
...
InfiniteScroller.prototype._setupAnimations = function(tombstoneAnimations, curPos) {
  ...
  for (var i in tombstoneAnimations) {
    ...
    this.items[i].node.style[style.transform] =
      'translateY(' +
      (this.anchorScrollTop + animation[1]) +
      'px) scale(' +
      this.tombstoneWidth / this.items[i].width +
      ', ' +
      this.tombstoneHeight / this.items[i].height +
      ')'
  ...
  for (var _i2 = this.firstAttachedItem; _i2 < this.lastAttachedItem; _i2++) {
    ...
    if (_animation) {
      ...
      tombstoneNode.style[style.transition] =
        cssVendor + 'transform ' + ANIMATION_DURATION_MS + 'ms, opacity ' + ANIMATION_DURATION_MS + 'ms'
      tombstoneNode.style[style.transform] =
        'translateY(' +
        curPos +
        'px) scale(' +
        this.items[_i2].width / this.tombstoneWidth +
        ', ' +
        this.items[_i2].height / this.tombstoneHeight +
        ')'
